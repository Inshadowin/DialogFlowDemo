...
if (jQuery) (function () {
    $.extend($.fn, {
        contextMenu: function (o, callback) {
            ...
            if (o.inSpeed == undefined) o.inSpeed = 10;
            ...
            $(this).each(function () {
                ...
                var menu = $('#' + o.menu).addClass('contextMenu');
                menu.appendTo("body");
                ...
                $(this).bind("mousedown.contextMenu", function (e) {
                    ...
                    $(this).bind("mouseup.contextMenu", function (e) {
                        ...
                        if (evt.button == 2 || evt.ctrlKey || o.openOnClick) {
                            ...
                            //console.log(o);
                            try {
                                if (o.onBeforeContextMenu)
                                    o.onBeforeContextMenu(srcElement, $('#' + o.menu), o);
                            ...
                            if (o.isIFrame) {
                                if (window.parent.$(".k-editable-area:visible").length > 0) {
                                    var offset = window.parent.$(".k-editable-area:visible").find("iframe").offset();
                                    y = y + offset.top;
                                    x = x + offset.left;
                                }
                                else {
                                    if ($(".k-editable-area:visible").length > 0) {
                                        var offset = $(".k-editable-area:visible").find("iframe").offset();
                                        y = y + offset.top;
                                        x = x + offset.left;
                                    }
                                }
                            ...
                            $('#' + o.menu).find('LI:not(.ui-state-disabled) A').bind("click.contextMenu", function () {
                                ...
                                if (callback) callback(action, $(srcElement), {}, $(this).attr("msg") || $(this).text());
                                return false;
                            });
                ...
                } else if (navigator.appName.indexOf("Explorer") > -1 || navigator.userAgent.toLowerCase().indexOf("trident") > -1) {
        ...
        disableByText: function (o) {
            if (o == undefined) {
                // Enable all
                $(this).find('LI').addClass('ui-state-disabled').css('display', 'none').css('visibility', 'hidden');
                return ($(this));
            }
            $(this).each(function () {
                if (o != undefined) {
                    var d = o.split(',');
                    for (var i = 0; i < d.length; i++) {
                        $(this).find('A[rel="' + d[i] + '"]').parent().addClass('ui-state-disabled').css('display', 'none').css('visibility', 'hidden');
                    }
                }
            });
            return ($(this));
        ...
        enableByText: function (o) {
            if (o == undefined) {
                // Enable all
                $(this).find('LI.ui-state-disabled').removeClass('ui-state-disabled').css('display', '').css('visibility', '');
                return ($(this));
            }
            $(this).each(function () {
                if (o != undefined) {
                    var d = o.split(',');
                    for (var i = 0; i < d.length; i++) {
                        $(this).find('A[rel="' + d[i] + '"]').parent().removeClass('ui-state-disabled').css('display', '').css('visibility', '');
                    }
                }
            });
            return ($(this));
